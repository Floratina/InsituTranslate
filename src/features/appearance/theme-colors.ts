...
function deriveCustomThemePalette(value: string): CustomThemePalette {
  ...
  return {
    ...
    dark: {
      ...
      "--border": oklchCss(clamp(darkBackgroundLightness + 0.17, 0.28, 0.36), Math.min(primaryChroma * 0.10, 0.026), hue),
      "--input": oklchCss(clamp(darkBackgroundLightness + 0.17, 0.28, 0.36), Math.min(primaryChroma * 0.10, 0.026), hue),
      "--ring": darkPrimary,
