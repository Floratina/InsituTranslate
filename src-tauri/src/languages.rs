...
pub fn normalize_language_code(value: &str) -> Option<&'static str> {
    ...
    match normalized.to_ascii_lowercase().as_str() {
        "simplified chinese" | "chinese simplified" | "zh-hans" | "zh-hans-cn" => Some("zh-CN"),
        "traditional chinese"
        | "chinese traditional"
...
pub fn normalize_target_language(value: &str) -> Result<String, String> {
    let code = normalize_language_code(value).ok_or_else(|| {
        "Target language must be selected from the supported language list".to_string()
    })?;
...
mod tests {
    ...
    fn normalizes_codes_and_legacy_names() {
        ...
        assert_eq!(
            normalize_language_code("Chinese (Traditional)"),
            Some("zh-HK")
        );
    ...
    fn maps_target_code_to_prompt_name() {
        assert_eq!(
            target_language_name("zh-CN").unwrap(),
            "Chinese (Simplified)"
        );
